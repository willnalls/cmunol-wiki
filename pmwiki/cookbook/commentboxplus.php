...
*/
# Version date
$RecipeInfo['CommentBoxPlus']['Version'] = '2020-02-26';
...
if($EnableCommentFormCheck==1) {
	...
	Markup('cboxcheck', '>block', '/\\(:cboxcheck:\\)/', "CommentBoxCheckMarkup");
	function CommentBoxCheckMarkup() {
		return Keep($GLOBALS['CommentBoxCheckFmt']);
	}
}
...
Markup('cbox','<links','/\(:commentbox(chrono)?(?:\\s+(\\S.*?))?:\)/', "CommentBoxMarkup");
function CommentBoxMarkup($m) {
	global $Chrono, $PostTo, $AccessCode, $CaptchaValue;
	extract($GLOBALS['MarkupToHTML']);
	return '<:block>'.Keep(str_replace(array('$Chrono','$PostTo','$AccessCode','$CaptchaValue'),
        		array($m[1], $m[2], RandomAccess(), CaptchaValueRelay()),
        		FmtPageName($GLOBALS['CommentBoxFmt'], $pagename)));
}

Markup('dbox','<block','/\(:diarybox:\)/',	"DiaryBoxMarkup");
function DiaryBoxMarkup() {
	extract($GLOBALS['MarkupToHTML']);
	return '<:block>'.str_replace('$AccessCode', RandomAccess(),
